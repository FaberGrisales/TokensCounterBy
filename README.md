...
**Limitación honesta:** esta opción **no** puede mostrar el porcentaje exacto de tu límite de plan usado ni la hora en que se reinicia tu ventana de 5 horas / semanal. Probé a estimarlo localmente (buscando huecos de inactividad en tus transcripts) pero el resultado era engañoso: cualquier pausa real de 5+ horas hacía que el contador pareciera "reiniciarse" a cero, aunque llevaras usando Claude Code todo el día — porque la ventana real de Anthropic es "rolling" (se recalcula continuamente, sin un único momento de reinicio) y no se puede modelar con precisión como una sesión con inicio/fin fijo. Investigué a fondo (`~/.claude.json`, `~/.claude/.credentials.json`, `~/.claude/policy-limits.json`) y confirmé que ese % y esa hora de reinicio no quedan cacheados en ningún archivo local — Anthropic los calcula en su servidor contra una cuota por tier que no es pública. Por eso esta opción se queda solo con "Recent Consumption": el consumo real, sin fingir saber el % ni la hora de reinicio. Solo el comando real `/usage` dentro de Claude Code puede mostrarte esos dos datos de forma autoritativa.

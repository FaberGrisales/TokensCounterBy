...
**Limitación honesta:** esta opción **no** puede mostrar el porcentaje exacto de tu límite de plan usado ni la hora exacta de reinicio que calcula Anthropic en su servidor contra una cuota por tier que no es pública. Investigué a fondo (`~/.claude.json`, `~/.claude/.credentials.json`, `~/.claude/policy-limits.json`) y confirmé que ese % y esa cuota no quedan cacheados en ningún archivo local. "Session Window Usage" es lo más cercano y honesto que puedo calcular con datos 100% locales; solo el comando real `/usage` dentro de Claude Code puede mostrarte el % y reinicio autoritativos de tu plan.

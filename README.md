...
- **Estimated Session Window** (nuevo): una barra de progreso y estimación de **cuánto tiempo llevas** en tu ventana actual de 5h/7 días, **cuánto tiempo te queda (estimado)**, y **a qué hora se reiniciaría (estimado)**. Se calcula buscando, en tus transcripts locales, el último hueco de inactividad más largo que la ventana misma (5h o 7 días) — el punto donde empezó "tu racha actual" — y proyectando desde ahí.
...
**Limitación honesta:** "Estimated Session Window" es una **estimación local**, no el dato real de Anthropic. El % exacto de tu límite de plan usado y la hora exacta de reinicio de tu ventana de 5 horas / semanal los calcula Anthropic en su servidor, contra una cuota por tier que no es pública — investigué a fondo (`~/.claude.json`, `~/.claude/.credentials.json`, `~/.claude/policy-limits.json`) y confirmé que ese dato no queda cacheado en ningún archivo local. Mi estimación puede equivocarse si, por ejemplo, estuviste inactivo en esta máquina pero activo en otra, o si Anthropic usa una lógica de ventana distinta a "racha continua de actividad". Solo el comando real `/usage` dentro de Claude Code puede mostrarte el % y la hora de reinicio autoritativos.
